...
This version supports two scan methods on the main scanner page:

1. **Picture / Camera Scan** - uses the device camera through the browser.
2. **Physical Scanner** - supports USB/Bluetooth barcode scanners that type the barcode into an input field and usually press Enter automatically.

Both modes only read the barcode. Inventory is not changed until the user reviews the action/quantity and clicks **Submit Add** or **Submit Remove**.
