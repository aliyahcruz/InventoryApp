...
export const tableNames = {
  ...
  barcodeMappings: process.env.AIRTABLE_BARCODE_MAPPINGS_TABLE || "Barcode Mappings",
  cycleCountLog: process.env.AIRTABLE_CYCLE_COUNT_LOG_TABLE || "Cycle Count Log",
};
...
export async function findUnknownScanById(id: string) {
...
export async function findBarcodeMappingByRawScan(rawScanValue: string) {
  const target = String(rawScanValue || "").trim();
  if (!target) return null;
  const records = await listRecords(tableNames.barcodeMappings);
  return records.find((record: any) =>
    String(record.fields["Raw Scan Value"] || "").trim() === target &&
