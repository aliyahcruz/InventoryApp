...
export async function POST(req: NextRequest) {
  try {
    ...
    // Critical fix:
    // Do NOT create another Inventory record when this NDC already exists.
    // This prevents multiple Inventory lines with the same NDC.
    ...
    // If the Inventory record already exists, do not overwrite its quantity.
    // Just mark this unknown scan as resolved and link it to the existing record.
    const finalNotes =
      resolutionNotes ||
      (createdNewInventory
