...
import { createRecord, findBarcodeMappingByRawScan, findInventoryByNdc, findUnknownScanById, tableNames, updateRecord } from "@/lib/airtable";
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!ndc) return NextResponse.json({ error: "A corrected NDC is required." }, { status: 400 });
    ...
    // Persist the correction so the exact raw barcode maps to this NDC on future scans.
    if (rawScanValue) {
      const existingMapping = await findBarcodeMappingByRawScan(rawScanValue);
      const mappingFields: Record<string, any> = {
      ...
      if (existingMapping) {
        await updateRecord(tableNames.barcodeMappings, existingMapping.id, mappingFields);
      } else {
        await createRecord(tableNames.barcodeMappings, mappingFields);
      }
    }
