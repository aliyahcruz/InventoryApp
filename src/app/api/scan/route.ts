  ...
  try {
    ...
    if (!ndc) {
      await createRecord(tableNames.unknownScans, {
        ...
        "Notes": notes || "Could not extract NDC from raw scan value.",
        "Resolved": false,
      });
      ...
      return NextResponse.json({
        success: false,
        unknownScan: true,
        message: "Could not extract NDC. Raw scan was logged for review.",
        rawScanValue,
        ndc: "",
      });
    }
    ...
    return NextResponse.json({
      ...
      rawScanValue,
      ndc,
      productName: Array.isArray(inventoryRecord.fields["Product"]) ? String(inventoryRecord.fields["Product"][0]) : undefined,
      previousQuantity,
      newQuantity,
      minimumQuantity,
      lowStock,
