...
export default function InventoryPage() {
  ...
  return (
    <main className="container">
      <h1>Inventory</h1>
      <p><a href="/">Back to Scanner</a> · <a href="/unknown-scans">Review Unknown Scans</a> · <a href="/reorder">Reorder / Low Stock</a></p>
            ...
            <table>
              ...
              <tbody>
                {filteredRecords.map((record) => (
                  <tr key={record.id} className={(Number(record.fields["Minimum Quantity"] || 0) > 0 && Number(record.fields["Current Quantity"] || 0) < Number(record.fields["Minimum Quantity"] || 0)) ? "low-stock-row" : ""}>
                    <td><strong>{record.fields["Item Name"] || "Unnamed Item"}</strong></td>
                    <td>{record.fields.Barcode || "—"}</td>
                    <td>{record.fields["Current Quantity"] ?? 0}{(Number(record.fields["Minimum Quantity"] || 0) > 0 && Number(record.fields["Current Quantity"] || 0) < Number(record.fields["Minimum Quantity"] || 0)) && <div className="low-stock-badge">LOW STOCK</div>}</td>
