...
export default function HomePage() {
  ...
  async function onScan(value: string) {
    ...
    // Look up the scan without changing inventory so the user can see
    // whether the barcode/NDC is already recognized before submitting.
    try {
      const lookupRes = await fetch("/api/cycle-count/lookup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rawScanValue: value }),
      });

      const lookup = await lookupRes.json();

      if (lookupRes.ok && lookup.found) {
        setDetectedNdc(lookup.ndc || suggested);
        setResult({
          recognizedScan: true,
          message: "Barcode recognized. Review the scan details below, then submit the inventory change.",
          rawScanValue: value,
          ndc: lookup.ndc || suggested,
      ...
      // Keep the initial red status card if lookup is unavailable.
    }
  }
  ...
  async function submitScan() {
    ...
    try {
      ...
      const res = await fetch("/api/scan", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ rawScanValue, ndc, action, quantity: Number(quantity), scannedBy, notes }),
      });
      ...
      // Clear Scan Details after a successful submission so the page is ready
      // for the next item while leaving the completed result visible above.
      setRawScanValue("");
      setDetectedNdc("");
      setAction("Remove");
      setQuantity("1");
    ...
    <main className="container">
      ...
      <div className={`card ${flash ? "scan-flash success" : ""}`}>
        ...
        <input
          ...
          placeholder="Review or correct NDC"
        />
        ...
        <button disabled={loading || !rawScanValue || !normalizedDetectedNdc || !quantity || Number(quantity) < 1} onClick={submitScan}>
