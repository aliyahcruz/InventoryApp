...
export default function HomePage() {
  ...
  async function submitScan(barcodeOverride?: string) {
    ...
    try {
      ...
      if (data.unknownScan) {
        ...
        setResult(data.item);
        setMessage(data.lowStock ? "⚠ Inventory updated, but this item is now BELOW MINIMUM. Scanner is still ready." : "Inventory updated and scan logged. Scanner is still ready.");
      }
      setNotes("");
    } catch (err) {
  ...
  return (
    <main className="container">
      ...
      <p><a href="/unknown-scans">Review Unknown Scans</a> · <a href="/inventory">View Inventory</a> · <a href="/reorder">Reorder / Low Stock</a></p>
      ...
      {result?.lowStock && (
        <div className="notice low-stock-alert">
          <strong>⚠ Below minimum stock level</strong>
          <div>{result.itemName || "This item"} is now at {result.newQuantity ?? 0}. Minimum quantity is {result.minimumQuantity ?? 0}.</div>
          <div><a href="/reorder">View Reorder / Low Stock page</a></div>
        </div>
      )}
