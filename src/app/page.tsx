...
export default function HomePage() {
  ...
  const ndc = useMemo(() => extractNdc(rawScanValue), [rawScanValue]);

  function onScan(value: string) {
    setRawScanValue(value);
    setResult({
      message: "Barcode read. Review the detected NDC, action, and quantity, then submit.",
      rawScanValue: value,
      ndc: extractNdc(value),
    });
    setFlash(true);
  ...
  async function submitScan() {
    ...
    try {
      const res = await fetch("/api/scan", {
        ...
        body: JSON.stringify({ rawScanValue, action, quantity, scannedBy, notes }),
      });
  ...
  return (
    <main className="container">
      ...
      <div className={`card ${flash ? "scan-flash success" : ""}`}>
        ...
        <label>Detected NDC</label>
        <input value={ndc} readOnly placeholder="Detected NDC will appear here" />
        ...
        <button disabled={loading || !rawScanValue || !ndc || quantity < 1} onClick={submitScan}>
      ...
      {result && (
        <div className={`card ${result.success ? "success" : result.unknownScan || result.lowStock ? "warning" : "error"}`}>
          ...
          {result.ndc && <p><strong>Detected NDC:</strong> {result.ndc}</p>}
