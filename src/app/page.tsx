...
export default function HomePage() {
  ...
  function changeScanMode(mode: ScanMode) {
    ...
    setScanMode(mode);
    setMessage(mode === "physical" ? "Physical scanner mode selected. Click the scanner input, then scan." : "Camera scan mode selected.");
    setTimeout(() => {
      if (mode === "physical") physicalInputRef.current?.focus();
    }, 50);
  }

  function handlePhysicalScannerSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    <main className="container">
      ...
      {scanMode === "physical" && (
        <div className={scanFlash ? "card flash-border" : "card"}>
          ...
          <p className="small">Most USB/Bluetooth barcode scanners work like a keyboard. Click the box below, then scan the item. The scanner usually sends Enter automatically.</p>
          <form onSubmit={handlePhysicalScannerSubmit}>
            <label>Physical scanner input</label>
            <input
              ref={physicalInputRef}
              value={physicalScannerValue}
              onChange={(e) => setPhysicalScannerValue(e.target.value)}
              placeholder="Click here, then scan with physical scanner"
              autoFocus
            />
            <button className="primary" type="submit" disabled={!physicalScannerValue.trim()}>
              Use Scanned Barcode
            </button>
          </form>
        </div>
