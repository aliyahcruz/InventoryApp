...
export default function HomePage() {
    ...
    try {
      const controls = await reader.decodeFromVideoDevice(undefined, videoRef.current!, (scanResult) => {
        ...
        if (autoSubmit) {
          ...
          setMessage(`Scanned barcode: ${text}. Scanner is still on. Review quantity, then submit.`);
        }
      });
  ...
  return (
    <main className="container">
      ...
      <p className="small">Scan a barcode, choose Add or Remove, enter quantity, then submit. The camera stays on until you press Stop Scanner.</p>
      ...
      <div className="card">
        ...
        <label className="checkline">
          ...
          Auto-submit each scan using the selected action and quantity
        </label>
        <p className="small">Leave auto-submit off if you want to confirm each quantity before updating Airtable.</p>
      </div>
      ...
      {unknownScan && (
        <div className="card warning">
          ...
          <p className="small">Inventory was not updated. Review this barcode in Airtable → Unknown Scans.</p>
        </div>
      )}
